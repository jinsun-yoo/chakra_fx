...
class NanoGptProfiler(ModelProfiler):
    def __init__(self, 
        ...
        if dse_config_filepath is not None:
            filename = os.path.basename(dse_config_filepath)  # Extract the filename
            filename_without_extension = os.path.splitext(filename)[0]  # Remove the extension
            #self.name = f"nanogpt_config_{filename_without_extension}"
            self.name = f"nanogpt"
            ...
            tp_model = parallelize_module(
                ...
                #     # "attn.c_attn": ColwiseParallel(),
                #     # "attn.c_proj": RowwiseParallel(),
                #     "mlp.c_fc": RowwiseParallel(input_layouts=Partial()),
                #     # "mlp.c_proj": RowwiseParallel(),
                # },
            )
