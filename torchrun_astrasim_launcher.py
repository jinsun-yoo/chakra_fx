...
def parse_args():
    ...
    parser.add_argument(
        ...
        default="",
        required=False,
        help="Path to the configuration file. Mutually exclusive with 'config_*' options below.",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default="",
        required=False,
        help="Path to the output directory where Chakra ET will be stored",
    )

    # Arguments defining the parallelization configuration.
    # TODO: Add more arguments as we expand search space.
    parser.add_argument(
        "--config_parallelization",
        type=str,
        default="",
        help="Parallelization strategy. Oneof: 'fsdp', 'tp', 'tp,fsdp'. Mutually exclusive with '--config_filepath'.",
    )
    parser.add_argument(
        "--config_tp",
        type=int,
        default=1,
        help="Degree of Tensor Parallelism. 'config_tp' x 'config_fsdp' is the total number of GPU.",
    )
    parser.add_argument(
        "--config_fsdp",
        type=int,
        default=1,
        help="Degree of FSDP Parallelism. 'config_tp' x 'config_fsdp' is the total number of GPU",
    )
...
# Sample Call: torchrun_astrasim_launcher.py --config_parallelization "tp,fsdp" --config_tp 4 --config_fsdp=2
# Each 'step' of the FxChakraAstrasimEnv in standalone Akasha will call this script.
# This script will extract the Chakra graph of an arbitrary configuration, and run it through ASTRA-sim.
# Ref: https://github.com/srivatsankrishnan/cloudaix/pull/1
